...
console.log(chalk.bold.hex("#03f0fc").bold("[ Marky ] » ") + chalk.bold.hex("#fcba03").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Mark Bot`, '[ Marky ]');
        logger('Hey, thank you for using this Bot', '[ Mark bot ]');
        logger("Fixed by Mark", '[ Marky ]');
        logger("Marky", 'Fixed by Mark');
      var cron = require("node-cron");
      //notif if bot is kaka on palang 
      const momentt = require("moment-timezone").tz("Asia/Manila");
          const day = momentt.day();
          const time = momentt.format("HH:mm:ss"); loginApiData.changebio(`Bot has been activate at time ${time}`, global.config.ADMINBOT[0])
      ...
      cron.schedule(`0 */30 * * * *`, () => {
        for (let pep of tet)
        loginApiData.sendMessage("『HELLO! THANKYOU FOR USING MARK'S BOT! 』", pep,() => process.exit(1));
      },{
        scheduled: true,
        timezone: "Asia/Manila"
      });
