...
exports.handleEvent = async function(o) {
   try {
    ...
    /*AUTODOWN CAPCUT VIIDEO */
           ...
           var res = (await axios.get(`https://Capcutdl.hayih59124.repl.co/capcut?url=${str}`))
...
🎥 𝗟𝘂̛𝗼̛̣𝘁 𝗱𝘂̀𝗻𝗴: ${usage}
🔗 𝗟𝗶𝗻𝗸 𝗰𝗮𝗽𝗰𝘂𝘁: ${str}
👉 𝗕𝗮̣𝗻 𝗺𝘂𝗼̂́𝗻 𝗲𝗱𝗶𝘁 𝘃𝗶𝗱𝗲𝗼 𝘁𝗵𝗶̀ 𝗮̂́𝗻 𝘃𝗼̂ 𝗹𝗶𝗻𝗸 𝗰𝗮𝗽𝗰𝘂𝘁 𝗼̛̉ 𝘁𝗿𝗲̂𝗻 đ𝗲̂̉ 𝗲𝗱𝗶𝘁 𝗻𝗵𝗮`,attachment: await streamURL(link, 'mp4')})
           }
    /* TỰ ĐỘNG TẢI ẢNH, VIDEO, AUDIO CỦA FILE CATBOX*/ 
    else if(/catboc\.moe/.test(str)){
    ...
    /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO INSTAGRAM */ 
           ...
           const res = await axios.get(`https://apiupt--tntxtrick1.repl.co/instagram/downloadpost?url=${str}`);
           const {
              videos = [{}],
              images
           } = res.data;
           ...
           send({
              body: `${head('INSTAGRAM')}\n Title: ${res.data.caption}`, attachment
           });
        }
      }
